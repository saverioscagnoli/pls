...
impl TemplateVariable {
    fn from_str(s: &str) -> Option<Self> {
        match s {
            "depth" => Some(TemplateVariable::Depth),
            "icon" => Some(TemplateVariable::Icon),
            "name" => Some(TemplateVariable::Name),
            "permissions" => Some(TemplateVariable::Permissions),
            "size" => Some(TemplateVariable::Size),
            "last_modified" => Some(TemplateVariable::LastModified),
            "git_status" => Some(TemplateVariable::GitStatus),
            "nlink" => Some(TemplateVariable::Nlink),
            "link_target" => Some(TemplateVariable::LinkTarget),
            _ => None,
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FormatTemplate(pub Vec<FormatPart>);

impl Deref for FormatTemplate {
    type Target = Vec<FormatPart>;
...
impl FormatTemplate {
    /// Parse a template string like "{icon} - {name}" into parts
    pub fn parse(template: &str) -> Self {
        ...
        while let Some(ch) = chars.next() {
            if ch == '{' {
                ...
                if found_closing {
                    if let Some(variable) = TemplateVariable::from_str(&var_name) {
                        parts.push(FormatPart::Variable(variable));
                    } else {
                        // Invalid variable, treat as literal
                        current_literal.push_str(&format!("{{{}}}", var_name));
                    }
                } else {
    ...
    pub fn default_template() -> Vec<Self> {
        vec![
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Depth)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Name)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Icon)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Permissions)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Size)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::LastModified)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::GitStatus)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::Nlink)]),
            FormatTemplate(vec![FormatPart::Variable(TemplateVariable::LinkTarget)]),
        ]
    }
}
...
pub struct LsConfig {
    ...
    #[serde_inline_default(FormatTemplate::default_template())]
    pub format: Vec<FormatTemplate>,
